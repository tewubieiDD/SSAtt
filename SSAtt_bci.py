...
from SSAtt.SSAtt import SSAtt_bci
...
if __name__=='__main__':
    ...
    ap.add_argument('--model_name', type=str, default='SSAtt_bci')
    ap.add_argument('--dataset', type=str, default='MI', help='dataset name')
    args = vars(ap.parse_args())

    set_seed_thread(args["repeat"])
    exp_path = create_exp_dir(args["output_dir"], args["sub"], args["model_name"], args["dataset"])
    ...
    trainloader, validloader, testloader = getAllDataloader(subject=args['sub'],
                                                            ...
                                                            bs=args['bs'])
    net = SSAtt_bci(args['epochs']).double().cpu()
