...
class SSAtt_bci(nn.Module):
    def __init__(self, epochs):
        ...
        self.att2 = SubmanifoldAttention([4, 9, 16], 4, 16)
    ...
    def forward(self, x):
        ...
        x2 = self.subcov(x[:, 1, :, :])
        x3 = self.subcov(x[:, 2, :, :])
        x1, shape = self.att2(x1)
        x1 = self.ract2(x1)
        x2, shape = self.att2(x2)
        x2 = self.ract2(x2)
        x3, shape = self.att2(x3)
        x3 = self.ract2(x3)
...
class SSAtt_mamem(nn.Module):
    def __init__(self, epochs):
        ...
        self.flat = nn.Flatten()
        # fc
        self.linear = nn.Linear(8 * 17, 5, bias=True)
    ...
    def forward(self, x):
        ...
        x3, shape = self.att2(x3)
        x3 = self.ract2(x3)
        x4, shape = self.att2(x4)
        x4 = self.ract2(x4)
        x5, shape = self.att2(x5)
        x5 = self.ract2(x5)
        x6, shape = self.att2(x6)
        x6 = self.ract2(x6)
        x7, shape = self.att2(x7)
        x7 = self.ract2(x7)
...
class SSAtt_cha(nn.Module):
    def __init__(self, epochs):
        ...
        self.flat = nn.Flatten()
        # fc
        self.linear = nn.Linear(8 * 17, 2, bias=True)
