...
from SSAtt import StiefelParameter
from SSAtt.rresnet.manifolds import SPD
from SSAtt.rresnet.manifolds.spd import _mvmt
...
class Res(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # vector field
        self.P = torch.empty(dim, dim, dtype=torch.float64)
        nn.init.normal_(self.P, std=1e-2)
        self.P = torch.svd(self.P)[0]
        self.P = StiefelParameter(self.P)
        self.manifold = SPD(metric="aff_inv")
        # self.spectrum_map = nn.Linear(dim, dim).double()
    ...
    def forward(self, x, y):
        ...
        # pt
        v1 = self.manifold.transp(x, y, v1)

        # exp
        out = self.manifold.exp(y, v1)
...
class SSAtt_bci(nn.Module):
    def __init__(self, epochs):
        ...
        # bs, 22, 1, sample
        self.conv2 = nn.Conv2d(22, 16, (1, 12), padding=(0, 6))
        ...
        self.matt = AttentionManifold(16, 14)

        # Res
        self.res = Res(16)

        # R2E
        self.tangent = SPDTangentSpace(14)
        self.flat = nn.Flatten()
        # fc
        self.linear = nn.Linear(7 * 15 * epochs, 4, bias=True)
    ...
    def forward(self, x):
        ...
        x2 = x[:, 1, :, :]
        x3 = x[:, 2, :, :]
        x1_slist = self.subcov(x1)
        x2_slist = self.subcov(x2)
        x3_slist = self.subcov(x3)
        x1_v, shape1 = self.satt(x1_slist)
        x1 = self.res(x1_v, x1)
        x2_v, shape2 = self.satt(x2_slist)
        x2 = self.res(x2_v, x2)
        x3_v, shape3 = self.satt(x3_slist)
        x3 = self.res(x3_v, x3)
...
class SSAtt_mamem(nn.Module):
    ...
    def forward(self, x):
        ...
        x2_slist = self.subcov(x2)
        x3_slist = self.subcov(x3)
        x4_slist = self.subcov(x4)
        x5_slist = self.subcov(x5)
        x6_slist = self.subcov(x6)
        x7_slist = self.subcov(x7)
        x1_v, shape1 = self.satt(x1_slist)
        x1 = self.res(x1_v, x1)
        x2_v, shape2 = self.satt(x2_slist)
        x2 = self.res(x2_v, x2)
        x3_v, shape3 = self.satt(x3_slist)
        x3 = self.res(x3_v, x3)
        x4_v, shape4 = self.satt(x4_slist)
        x4 = self.res(x4_v, x4)
        x5_v, shape5 = self.satt(x5_slist)
        x5 = self.res(x5_v, x5)
        x6_v, shape6 = self.satt(x6_slist)
        x6 = self.res(x6_v, x6)
        x7_v, shape7 = self.satt(x7_slist)
        x7 = self.res(x7_v, x7)
